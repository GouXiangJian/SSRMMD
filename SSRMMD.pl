...
Options:
  ...
  -me | -method <STR> : Algorithm for exactly checking flanking sequences
                        ...
                        Note: 'NO' mean that flanking sequences are perfectly
...
die "Error: option '-e'  just be set to 0 or 1 !\n" if $miningMethod != 0 and $miningMethod != 1;
...
if ($poly == 0) {
	...
	my $all_SSRs = multithreads_find_SSR_and_info($miningMethod, $molen_minum, $tmp_dir_name1, $final_threads_num, $fs_len, $minSsrLen, $maxSsrLen);
...
#07 getMotifInfo
#08 is_false_motif 
#09 miningSsrBySimple
#10 miningSsrByIntegrate
#11 multithreads_find_SSR_and_info
#12 some_simple_statistics
#13 generate_SSRs_with_fs_stat_file
#14 generate_fs_list
#15 keep_fs_uniq_SSRs
#16 keep_fs_cons_SSRs
#17 getRestSSRsFromUniSet
#18 prealign_highly_conservativeness
#19 mergeScalar
#20 mapFsToFindConSSR
#21 multiThreadsMapFsToFindConSSR
#22 splitRestSSRsToTempFiles
#23 Levenshtein_Distance
#24 Needleman_Wunsch
#25 generate_cons_or_uniq_SSRs_stat_file
#26 split_id_seq_values_to_diff_file
#27 get_fs_match_count
#28 sldingWindows
#29 multithreads_keep_unique_SSRs
#30 final_comparison_table
...
#function7 : store the motif length and minimum number of repeat into a hash, and return a hash reference.
sub getMotifInfo {
...
#function8 : judge if motif is false, 1 = false, 0 = true (example: 'ATAT', 'AAAA' and 'GAAGAA' is false).
sub is_false_motif {
    my $motif = shift;
...
#function9 : mining SSR loci by simple regular expression, and return a hash reference.
...
#function10 : mining SSR loci by integrated regular expression, and return a hash reference.
...
#function11 : use multi-threads to mine SSRs and their information, and return a hash reference.
...
#function12 : output some simple statistics about all SSRs loci.
...
#function13 : output a statistical file that contains SSRs(add flanking sequences) information to the current working directory.
...
#function14 : return a hash reference that key is the flanking sequence, value is the frequency of existence.
...
#function15 : return a hash reference that contains the unique SSRs of flanking sequences and their information.(compared with flanking sequences of all SSRs)
...
#function16 : return three hash references that contain conservative SSRs in each file and conservative SSRs statistical table, respectively.
...
#function17 : exclude SSR that already is conservative SSR, and return a hash reference.
...
#function18 : pre-alignment by highly conservativeness, and return a hash reference.
...
#function19 : merge two scalar into a single scalar, and return a hash reference.
sub mergeScalar {
    my ($one, $two) = @_;
...
#function20 : mapping two seqs for finding conservative SSRs, and return a array reference.
...
#function21 : use multithreads to map two seqs for finding conservative SSRs, and return three hash references.
...
#function22 : split hash into smaller hashes, and put these smaller hashes in temporary files.
...
#function23 : global alignment by using levenshtein distance.
...
#function24 : global alignment by using Needleman-Wunsch algorithm
...
#function25 : output a statistical file that contains conservative or unique SSRs to the current working directory.
...
#function26 : the values of hash id_seq are connected together and stored in a scalar, and this scalar are divided equally into smaller scalars, finally, put these smaller scalars in temporary files, and return the directory name created.
...
#function27 : return a array reference that contains match count of each flanking sequence compared with genome sequence.
...
#function28 : check uniqueness of flanking sequences by slding window.
sub sldingWindows {
...
#function29 : use multithreads and call function get_fs_match_count to keep the unique SSRs(the flanking sequence is unique) in the genome, and return a hash reference.
...
#function30 : output a statistical table that contains polymorphic SSRs candidate to the current working directory.
