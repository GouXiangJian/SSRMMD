...
sub find_SSR_and_info{
	...
	foreach my $id (sort keys %$id_seq){
		...
		push @{$id_info{$id}}, ['NA', 'NA', 'NA', 'NA', 'NA', 'NA', -1000];
		foreach my $short_seq (sort { $shseq_loci{$a} <=> $shseq_loci{$b} } keys %shseq_loci){
			...
			foreach my $molen (sort {$a <=> $b} keys %$molen_minum) {
				...
				while(1){
					...
					push @{$id_info{$id}}, [$id, $motif, $molen, $rep_num, $SSR_len, $start, $end] if (! &is_false_motif($motif)) and (abs($end-$id_info{$id}[-1][-1]) > $molen-1);
					$front_len += $cutlen;
				}
				$short_seq = $seq_tmp;
				$front_len = $len_tmp;
	...
	return \%id_info;
	#notice : Although we have taken remedial measures, there is still a case that
	####### : a very long SSR(30,000 bp) being cut off, but this kind of situation
	####### : is quite rare so we no need to worry about it.
}
