...
#Module2 : developing candidate polymorphic SSR
	...
	foreach my $fa_file_name ($fasta1_name, $fasta2_name) {
		...
		my ($tmp_dir_name1, $final_threads_num, $id_len) = dealSeqs($fa_file_name, $outDirName, $cwd, $threads_num);
		...
		#4 store SSR information to tmp file
	...
	my $fs_uniq_SSRs1 = multithreadsKeepUniqueSsr($tmp_dir_name3_1, $fs_con_SSRs_file1, $fs_len);
	...
	my $fs_uniq_SSRs2 = multithreadsKeepUniqueSsr($tmp_dir_name3_2, $fs_con_SSRs_file2, $fs_len);
...
sub dealSeqs {
	...
	die "Error: the format of file '$fileName' may be incorrect !\n" if $name !~ /\A>/;
	$name =~ s/[\r\n]+//; #filter CR/LF at the end
	$name =~ s/\A>//;     #filter > at the start
	$name =~ s/\s/_/g;    #replace all spaces by '_'
	my $seqLength;
...
sub readSeqs {
	...
	while (<$in>) {
		s/[\r\n]+//;
		if (/\A>/) {
			if ($seqTotal{$thread} >= 10_000_000) { #each tmp file must be >= 10 Mb
...
#function9 : judge if motif is false, 1 = false, 0 = true (example: 'ATAT', 'AAAA' and 'GAAGAA' is false).
sub is_false_motif {
    my $motif = shift;
...
sub multithreadsKeepUniqueSsr {
	...
	if (@files == 1) {
		my $match_count = getFsMatchCount($files[0], $new_fs_set, $fs_len);
		undef $new_fs_set;
		push @all_count, $match_count;
	}
	elsif (@files >= 2) {
		foreach my $file (@files) {
			my $thr = threads->create(\&getFsMatchCount, $file, $new_fs_set, $fs_len);
		}
		undef $new_fs_set;
		while (threads->list()) {
    		foreach my $thr (threads->list(threads::joinable)) {
