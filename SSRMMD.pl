...
Options: 
  ...
  -d |-detail  <int> : 0 = output intermediate file, 1 = not output (default:0)
  -s |-stat    <int> : 0 = output statistical  file, 1 = not output (default:0)
...
if($poly == 0){
	...
	&stat_SSR_num_in_each_motif($fa_base_name, $all_SSRs) if $stat_file;
