...
die "Error: option '-c'  must be a bigger than 0 !\n" if $thrIncreaseFold < 0;
...
#Module2 : developing candidate polymorphic SSR
	...
	foreach my $fa_file_name ($fasta1_name, $fasta2_name){
		...
		if ($conserMap ne 'NO') {
			...
			#get rest information of SSR that need to mapping by LD/NW

			my $restSSRs = getRestSSRs($fs_uni_SSRs1, $fs_con_SSRs1, $fs_uni_SSRs2, $fs_con_SSRs2, $fsReduceFold);
			...
			#split information to tmp file

			my $base_name = basename $fasta1_name;

			my $tmp_dir_name2 = splitRestSSRsToTempFiles($restSSRs, $threads_num, $base_name);

			undef $restSSRs;


			#start to conservativeness mapping by LD or NW

			my ($con_SSRs1, $con_SSRs2, $compare) = multiThreadsMapFsToFindConSSR($tmp_dir_name2, $conserThreshold, $thrIncreaseFold, $conserMap);


			#get conservativeness fasta1 file
			...
			#get conservativeness fasta2 file
			...
			#get conservativeness compare file
		...
		#output conservativeness file for debug
		...
		my $id_seq1 = $genome1_name eq $fasta1_name ? RetrieveFromFile($retrieve_files[0]) : &deal_fa_file($genome1_name, $outDirName, $cwd);

		my $tmp_dir_name3_1 = &split_id_seq_values_to_diff_file($id_seq1, $threads_num, $fa_base_name1);

		undef $id_seq1;

		my $fs_uniq_SSRs1 = &multithreads_keep_unique_SSRs($tmp_dir_name3_1, $fs_con_SSRs_file1, $fs_len, $fa_base_name1);
		...
		my $id_seq2 = $genome2_name eq $fasta2_name ? RetrieveFromFile($retrieve_files[2]) : &deal_fa_file($genome2_name, $outDirName, $cwd);

		my $tmp_dir_name3_2 = &split_id_seq_values_to_diff_file($id_seq2, $threads_num, $fa_base_name2);

		undef $id_seq2;

		my $fs_uniq_SSRs2 = &multithreads_keep_unique_SSRs($tmp_dir_name3_2, $fs_con_SSRs_file2, $fs_len, $fa_base_name2);
		...
		#retrieve conservativeness information of compare
...
#RetrieveFromFile
#deal_fa_file
#split_id_seq_to_diff_file
#average_allocation_algorithm
#get_motifs_info
#is_false_motif
#find_SSR_and_info
#multithreads_find_SSR_and_info
#some_simple_statistics
#generate_SSRs_with_fs_stat_file
#generate_fs_list
#keep_fs_uniq_SSRs
#keep_fs_cons_SSRs
#mergeScalar
#mapFsToFindConSSR
#multiThreadsMapFsToFindConSSR
#splitRestSSRsToTempFiles
#getRestSSRs
#Levenshtein_Distance
#Needleman_Wunsch
#generate_cons_or_uniq_SSRs_stat_file
#split_id_seq_values_to_diff_file
#get_fs_match_count
#multithreads_keep_unique_SSRs
...
#function16 : merge two scalar into a single scalar, and return a hash reference.
sub mergeScalar {
    my ($one, $two) = @_;
...
#function17 : map two seqs for finding conservative SSRs, and return a array reference.
sub mapFsToFindConSSR {
    my ($file, $conserThreshold, $incFold, $method) = @_;
    my $restSSRs = retrieve $file;
	my ($codeRef, $shortThreshold);
	if ($method eq 'LD') {
		$codeRef = \&Levenshtein_Distance;
		$shortThreshold = $conserThreshold*$incFold;
		$shortThreshold = 0.5 if $shortThreshold > 0.5;
	}
	elsif ($method eq 'NW') {
		$codeRef = \&Needleman_Wunsch;
		$shortThreshold = 1-(1-$conserThreshold)*$incFold;
		$shortThreshold = 0.5 if $shortThreshold < 0.5;
	}
    my (%con_SSRs1, %con_SSRs2, %compare);
    foreach my $motif (sort keys %$restSSRs) {
        foreach my $SSR1 (@{$restSSRs->{$motif}{S1}}) {
            foreach my $SSR2 (@{$restSSRs->{$motif}{S2}}) {
                my ($judge1, undef, undef) = $codeRef->($SSR1->[9], $SSR1->[10], $SSR2->[9], $SSR2->[10], $shortThreshold); # map left seq
                if ($judge1) {
                    my ($judge2, undef, undef) = $codeRef->($SSR1->[13], $SSR1->[14], $SSR2->[13], $SSR2->[14], $shortThreshold); # map right seq
                    if ($judge2) {
                        my ($judge3, $Lresult, $Lwhich) = $codeRef->($SSR1->[7], $SSR1->[8], $SSR2->[7], $SSR2->[8], $conserThreshold); # map left seq
                        if ($judge3) {
                            my ($judge4, $Rresult, $Rwhich) = $codeRef->($SSR1->[11], $SSR1->[12], $SSR2->[11], $SSR2->[12], $conserThreshold); # map right seq
                            if ($judge4) {
                                my $left_fs1  = join '', @{$SSR1->[7]};
                                my $right_fs1 = join '', @{$SSR1->[11]};
                                my $fs1 = $left_fs1.'-'.$right_fs1;
                                $con_SSRs1{$fs1} = [ @{$SSR1}[0 .. 6] ];
                                my $left_fs2  = join '', @{$SSR2->[7]};
                                my $right_fs2 = join '', @{$SSR2->[11]};
                                my $fs2 = $left_fs2.'-'.$right_fs2;
                                $con_SSRs2{$fs2} = [ @{$SSR2}[0 .. 6] ];
                                $compare{$fs1} = [ @{$con_SSRs1{$fs1}}, @{$con_SSRs2{$fs2}}, $fs2, $Lresult, $Lwhich, $Rresult, $Rwhich ];
                                last;
                            }
                        }
                    }
...
#function18 : use multithreads to map two seqs for finding conservative SSRs, and return three hash references.
sub multiThreadsMapFsToFindConSSR {
	my ($tmp_dir_name, $conserThreshold, $incFold, $method) = @_;
	foreach my $file (glob "$tmp_dir_name/*"){
		my $thr = threads->create(\&mapFsToFindConSSR, $file, $conserThreshold, $incFold, $method);
	}
	my (%con_SSRs1, %con_SSRs2, %compare);
	while(threads->list()){
    	foreach my $thr (threads->list(threads::joinable)){
...
#function19 : split hash restSSRs into smaller hashes, and put these smaller hashes in temporary files.
sub splitRestSSRsToTempFiles {
    ...
    if ($threads_num == 1) {
        ...
        delete $restSSRs->{total};
        store $restSSRs, $full_name;
    }
    else {
        my $eachThreadSsrNum = int($restSSRs->{total}/$threads_num) + ($restSSRs->{total} % $threads_num ? 1 : 0); 
        delete $restSSRs->{total};
        my %outThreadSSRs;
        my $outSum = 0;
        my $count = 1;
        foreach my $motif (sort {$restSSRs->{$b}{N1} <=> $restSSRs->{$a}{N1}} keys %$restSSRs) {
            my $sum = @{$restSSRs->{$motif}{S1}};
            next if $sum == 0;
            if ($eachThreadSsrNum - $outSum > $sum) {
                $outThreadSSRs{$motif} = $restSSRs->{$motif};
                $outSum += $sum;
            }
            else {
                if ($eachThreadSsrNum != $outSum) {
                    $outThreadSSRs{$motif}{S2} = $restSSRs->{$motif}{S2};
                    $outThreadSSRs{$motif}{S1} = [ @{$restSSRs->{$motif}{S1}}[0 .. $eachThreadSsrNum-$outSum-1] ];
                    splice @{$restSSRs->{$motif}{S1}}, 0, $eachThreadSsrNum-$outSum;
                }
                my $full_name = File::Spec->catfile($tmp_dir_name, "thread.$count");
...
#function20 : return a hash references that contain rest non-conservative SSRs.
sub getRestSSRs {
    my ($fs_uni_SSRs1, $fs_con_SSRs1, $fs_uni_SSRs2, $fs_con_SSRs2, $reduceFactor) = @_;
    my %restSSRs;
    my $count = 1;
    foreach my $fs_uni_SSRs ($fs_uni_SSRs1, $fs_uni_SSRs2) {
        my $fs_con_SSRs = $count == 1 ? $fs_con_SSRs1 : $fs_con_SSRs2;
        my $whichOne = $count == 1 ? 'S1' : 'S2';
        $count++;
        foreach my $fs (sort keys %$fs_uni_SSRs) {
		    if (! exists $fs_con_SSRs->{$fs}) {
                my $motif = $fs_uni_SSRs->{$fs}[1];
                my ($left_fs, $right_fs) = split /-/, $fs;
                next unless $left_fs and $right_fs;
                my $leftFs = [ split //, $left_fs ];
                my $leftFsLen = @$leftFs;
				my $leftIntLen = int($leftFsLen*$reduceFactor);
				my $leftFsReduceLen = $leftIntLen + ($leftFsLen*$reduceFactor > $leftIntLen ? 1 : 0);
                my $leftFsReduce = [ @$leftFs[ -$leftFsReduceLen .. -1] ];
                my $rightFs = [ split //, $right_fs ];
                my $rightFsLen = @$rightFs;
				my $rightIntLen = int($rightFsLen*$reduceFactor);
				my $rightFsReduceLen = $rightIntLen + ($rightFsLen*$reduceFactor > $rightIntLen ? 1 : 0);
                my $rightFsReduce = [ @$rightFs[ 0 .. $rightFsReduceLen-1] ];
                push @{$restSSRs{$motif}{$whichOne}}, [ @{$fs_uni_SSRs->{$fs}}, $leftFs, $leftFsLen, $leftFsReduce, $leftFsReduceLen, $rightFs, $rightFsLen, $rightFsReduce, $rightFsReduceLen ];
		    }
	    }
    }
    my $total = 0;
    foreach my $motif (sort keys %restSSRs) {
        if (keys %{$restSSRs{$motif}} != 2) {
            delete $restSSRs{$motif};
            next;
        }
        $restSSRs{$motif}{N1} = @{$restSSRs{$motif}{S1}};
        $total += @{$restSSRs{$motif}{S1}};
    }
    $restSSRs{total} = $total;
    undef $fs_uni_SSRs1;
    undef $fs_uni_SSRs2;
    undef $fs_con_SSRs1;
    undef $fs_con_SSRs2;
...
#function21 : global alignment by using levenshtein distance.
sub Levenshtein_Distance {
    my ($seq1, $seq1Len, $seq2, $seq2Len, $threshold) = @_;
    my @matrix_score;
    $matrix_score[0][0] = 0;
...
#function22 : global alignment by using Needleman-Wunsch algorithm
...
#function23 : output a statistical file that contains conservative or unique SSRs to the current working directory.
...
#function24 : the values of hash id_seq are connected together and stored in a scalar, and this scalar are divided equally into smaller scalars, finally, put these smaller scalars in temporary files, and return the directory name created.
...
#function25 : return a array reference that contains match count of each flanking sequence compared with genome sequence.
...
#function26 : use multithreads and call function get_fs_match_count to keep the unique SSRs(the flanking sequence is unique) in the genome, and return a hash reference.
sub multithreads_keep_unique_SSRs{
	my ($tmp_dir_name3, $fs_con_SSRs_file, $fs_len, $fa_base_name) = @_;
	...
	undef $fs_set;
	foreach my $file (glob "$tmp_dir_name3/*"){
...
#function27 : output a statistical table that contains polymorphic SSRs candidate to the current working directory.
