...
Function:
  Find out SSR loci and polymorphic SSR candidate
...
Options: 
  ...
  -f2|-fasta2  <str> : FASTA file (essential if need to find polymorphic site)
...
if($poly == 0){
	...
	&some_simple_statistics($fa_base_name, $all_SSRs, $id_seq) if $stat_file;
...
sub split_id_seq_to_diff_file{
	...
	my $tmp_dir_name1 = $fa_base_name.'.SSRMMD_tmp1';
	if(-e $tmp_dir_name1){
		chdir $tmp_dir_name1;
		unlink glob '*';
		chdir '..';
...
sub some_simple_statistics{
	...
	print OUT_ALL "======================================\n\n";
	print OUT_ALL "Seq_id\tSeq_length(bp)\tSSR_number\tSSR_density(No./Kb)\n";
	my $id_total = 0;
	my $len_total = 0;
	foreach my $id (sort keys %id_sum) {
		$id_total += $id_sum{$id};
		$len_total += $id_len{$id};
		print OUT_ALL "$id\t$id_len{$id}\t$id_sum{$id}\t";
		printf OUT_ALL "%.3f\n", ($id_sum{$id}*1000)/$id_len{$id};
	}
	print OUT_ALL "total\t$len_total\t$id_total\t";
	printf OUT_ALL "%.3f\n\n\n", ($id_total*1000)/$len_total;
	print OUT_ALL "2. Number of SSRs loci of per length of motif\n";
	print OUT_ALL "===================================\n\n";
	print OUT_ALL "Motif_length(bp)\tSSR_number\n";
	my $motif_total = 0;
	foreach my $motif_len (sort keys %motif_sum) {
		$motif_total += $motif_sum{$motif_len};
		print OUT_ALL "$motif_len\t$motif_sum{$motif_len}\n";
	}
	print OUT_ALL "total\t$motif_total\n\n\n";
...
sub split_SSR_and_id_seq_to_diff_file{
	...
	my $tmp_dir_name2 = $fa_base_name.'.SSRMMD_tmp2';
	if(-e $tmp_dir_name2){
		chdir $tmp_dir_name2;
		unlink glob '*';
		chdir '..';
...
sub split_id_seq_values_to_diff_file{
	my ($id_seq, $threads_num, $fa_base_name) = @_;
	my $tmp_dir_name3 = $fa_base_name.'.SSRMMD_tmp3';
	if(-e $tmp_dir_name3){
		chdir $tmp_dir_name3;
		unlink glob '*';
		chdir '..';
