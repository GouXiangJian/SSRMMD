...
#Module2 : developing candidate polymorphic SSR
	...
	foreach my $fa_file_name ($fasta1_name, $fasta2_name) {
		...
		#store conservativeness information to tmp file
		...
		#retrieve uniqueness information
...
sub find_SSR_and_info {
	...
	foreach my $file (@$files) {
		...
		foreach my $id (sort keys %$id_seq) {
			#cut each sequence into several short sequences of 1Mb in length
			...
			foreach my $i (0 .. $#shseq_loci) {
				...
				foreach my $molen (sort {$a <=> $b} keys %$molen_minum) {
					...
					while ($shseq_loci[$i][1] =~ /(([ATCG]{$molen})\g{-1}{$remain,})/g) {
						my ($SSR, $motif) = ($1, $2);
						my $SSR_len = length $SSR;
						my $rep_num = $SSR_len / $molen;
						my $start = $shseq_loci[$i][0] - 1 + $-[0] + 1;
						my $end   = $shseq_loci[$i][0] - 1 + $+[0];
						pos($shseq_loci[$i][1]) += 1 - $molen; #backtrack
						last if $start - $shseq_loci[$i][0] + 1 >= $supple_start;
	...
	undef %id_info;
	return \%all_SSRs;
	#notice : Although we have taken remedial measure, there is still 
