...
#You can also set more primer design conditions, in script line 248-262.
...
###############################################################################
Name:
  connectorToPrimer3 - Provide connection between SSRMMD and Primer3 (v2.5.0)

Author:
  Xiangjian Gou (xjgou\@stu.sicau.edu.cn)
  Haoran Shi (542561234\@qq.com)

Usage:
  perl connectorToPrimer3.pl opt1 <value1> opt2 <value2> ... optN <valueN>

Options:
  -i  | -input  <STR> : provide an input file name, only accept output file
  ...
  -s  | -source <INT> : the source of input (option '-i') file. (default: 1)
                           1 : from SSRMMD when only mining perfect SSR loci 
                           2 : from SSRMMD when further mining polymorphic SSRs
                           3 : from Primer3
  ...
  -f  | -filter <INT> : if option '-s'=2, filter input file. (default: 0)
                           0 : no filter
                           1 : only deal polymorphic SSRs (col 20 = 'yes')
                           2 : only deal distance(LD) = 0 (col 14,18 = '0.000')
                           3 : only deal identity(NW) = 1 (col 15,19 = '1.000')
                           4 : meet both 1 and 2
                           5 : meet both 1 and 3
...
Example:
  1. Design primers for all SSRs (SSRMMD option '-p' = 0):
       perl connectorToPrimer3.pl -i example1.fa.SSRs

  2. Design primers for candidate polymorphic SSRs (SSRMMD option '-p' = 1):
       perl connectorToPrimer3.pl -i example1.fa-and-example2.fa.compare -s 2
###############################################################################
...
sub parseSSRMMD {
    ...
    while (<$in>) {
        ...
        s/[\r\n]+//;
        my @row = split /\t/;
        die "Error: when option '-s' = 1, the input file '$inFile'(row $count) have a wrong format !\n" if @row != 12 and $inFileSource == 1;
        die "Error: when option '-s' = 2, the input file '$inFile'(row $count) have a wrong format !\n" if @row != 20 and $inFileSource == 2;
        next if /\Anumber/;
...
sub parsePrimer3 {
    ...
    print $out join("\t", qw/id forward_primer forward_length forward_TM forward_GC reverse_primer reverse_length reverse_TM reverse_GC produce_size/), "\n";
    while (<$in>) {
